  ...
  switch (type) {
    ...
    case "leadForm":
      return {
        ...
        description:
          "Collect buyer and seller inquiries and send them directly into your CRM.",
        buttonLabel: "Get My Free Consultation",
        disclaimer:
          "By submitting, you agree to be contacted about real estate opportunities and listings.",
...
${widget.data.items
  .map((colItems, idx) => {
    const span =
      (widget.data.spans && widget.data.spans[idx]) ||
      Math.floor(12 / widget.data.colCount);
...
function LeadFormFunctional({
    ...
    try {
      const response = await fetch("/api/v1/add/lead", {
        ...
        body: JSON.stringify({
          ...
          sourcePage:
            typeof window !== "undefined" ? window.location.href : "preview",
          lead: {
            name: formState.name,
            email: formState.email,
            phone: formState.phone,
  ...
  return (
    <div className="rounded-xl bg-white p-8 text-slate-900 shadow-inner">
      <form onSubmit={handleSubmit} className="space-y-5">
        <div
          className={`grid gap-5 ${isNested || isMobileView ? "grid-cols-1" : "sm:grid-cols-2"}`}
        >
        ...
        <button
          ...
          className="mt-4 transition-all duration-300 flex w-full items-center justify-center rounded-lg px-6 py-4 text-sm font-bold shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
          style={{
            backgroundColor: widget.data.buttonColor,
            color: widget.data.buttonTextColor,
          }}
        >
          {isSubmitting ? "Sending..." : widget.data.buttonLabel}
        </button>
        {submitStatus === "success" && (
          <p className="text-xs font-semibold text-emerald-500 text-center animate-in fade-in slide-in-from-top-1">
            {submitMessage}
          </p>
        )}
        {submitStatus === "error" && (
          <p className="text-xs font-semibold text-rose-500 text-center animate-in fade-in slide-in-from-top-1">
            {submitMessage}
          </p>
        )}
...
function renderWidget(
  ...
  switch (widget.type) {
            ...
            onChange={(event) =>
              onInlineUpdate(widget.id, (current) =>
                current.type !== "title"
                  ...
                  : {
                      ...current,
                      data: { ...current.data, text: event.target.value },
                    },
              )
            }
    ...
    case "image":
      return widgetWrapper(
        <div
          className={`${aspectClass(widget.data.aspect)} overflow-hidden rounded-xl shadow-md border border-slate-100 bg-slate-50 transition-transform duration-300 hover:scale-[1.01]`}
        >
          <img
    ...
    case "gallery":
      return widgetWrapper(
        <div
          className={`grid gap-6 ${isMobileView ? "grid-cols-1" : "sm:grid-cols-2 lg:grid-cols-3"}`}
        >
        ...
        <div className="overflow-hidden rounded-sm border border-[#d8dde3] bg-white">
          <div className="border-b border-[#e2e6ea] px-5 py-3">
            {onInlineUpdate ? (
              ...
              <p className="text-sm font-semibold uppercase tracking-[0.24em] text-[#83919d]">
                {widget.data.title}
              </p>
            )}
          </div>
          <div
            className={`grid gap-3 p-4 ${isMobileView ? "grid-cols-1" : "sm:grid-cols-3"}`}
          >
            {widget.data.images.map((image, index) => (
              <div
                key={`${image}-${index}`}
                className="aspect-[16/10] overflow-hidden rounded-sm bg-[#eef2f5]"
              >
                <img
                  src={image}
                  alt={`Slide ${index + 1}`}
    ...
    case "map":
      return widgetWrapper(
        <div
          className={`grid gap-8 rounded-2xl border border-slate-100 bg-white p-8 shadow-md ${isMobileView || isNested ? "grid-cols-1" : "lg:grid-cols-[0.4fr_0.6fr]"}`}
        >
          <div className="flex flex-col justify-center space-y-4">
            {onInlineUpdate ? (
              <input
    ...
    case "leadForm":
      return widgetWrapper(
        <div
          className={`grid gap-12 rounded-2xl border border-slate-100 ${isMobileView ? "p-6" : "p-10 shadow-xl"} ${isMobileView || isNested ? "grid-cols-1" : "lg:grid-cols-[0.9fr_1.1fr]"}`}
          style={{
        ...
        >
          <div className="flex flex-col justify-center space-y-6">
            <div>
              <p className="text-sm font-bold uppercase tracking-[0.24em] opacity-60">
                Lead capture
              </p>
              {onInlineUpdate ? (
                <input
                ...
                <h2
                  className={`mt-4 ${isNested || isMobileView ? "text-2xl" : "text-4xl"} font-bold tracking-tight leading-tight`}
                >
                  {widget.data.title}
                </h2>
              )}
          ...
          {onInlineUpdate ? (
            <div className="rounded-xl bg-white p-8 text-slate-900 shadow-inner">
              ...
              >
                <div
                  className={`${isNested || isMobileView ? "" : "sm:col-span-2"}`}
                >
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">
                    Full Name
                  </label>
                  <div className="w-full rounded-lg border border-slate-100 bg-slate-50 px-4 py-3.5 text-sm text-slate-400">
                    Your Name
                  </div>
                </div>
                <div>
              ...
              <button
                type="button"
                className="mt-8 transition-all duration-300 flex w-full items-center justify-center rounded-lg px-6 py-4 text-sm font-bold shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:scale-95"
                style={{
                  backgroundColor: widget.data.buttonColor,
                  color: widget.data.buttonTextColor,
                }}
                    ...
                    onChange={(e) =>
                      onInlineUpdate(widget.id, (c) =>
                        c.type === "leadForm"
                          ? {
                              ...c,
                              data: { ...c.data, buttonLabel: e.target.value },
                            }
                          : c,
                      )
                    }
                  ...
                  onChange={(e) =>
                    onInlineUpdate(widget.id, (c) =>
                      c.type === "leadForm"
                        ? {
                            ...c,
                            data: { ...c.data, disclaimer: e.target.value },
                          }
                        : c,
                    )
                  }
    ...
    case "divider":
      return widgetWrapper(
        <div
          ...
          style={{ backgroundColor: widget.data.color }}
        />,
      );
    case "spacer":
      ...
      return widgetWrapper(
        <div className="overflow-hidden rounded-sm border border-[#d8dde3] bg-white p-4">
          {onInlineUpdate ? (
            ...
            <div className="mb-3 text-sm font-semibold uppercase tracking-[0.24em] text-[#83919d]">
              {widget.data.title}
            </div>
          )}
  ...
  const renderPreviewWidget = (widget: CanvasWidget, isNested?: boolean) => {
    if (widget.type === "columns") {
      ...
      return (
        <div
          style={bgStyle}
          className={`${backgroundType !== "none" ? "rounded-xl overflow-hidden" : ""}`}
        >
          {widgetWrapper(
            <div
              className={`grid gap-6 md:gap-8 ${isMobileView ? "grid-cols-1" : getResponsiveGridClass(widget.data.colCount)}`}
            >
              {widget.data.items.map((col, cIdx) => {
                const span =
                  (widget.data.spans && widget.data.spans[cIdx]) ||
                  Math.floor(12 / widget.data.colCount);
                return (
                  <div
                    key={cIdx}
                    className={`flex flex-col gap-6 ${isMobileView ? "col-span-1" : getSpanClass(span)}`}
                  >
                    {col.map((inner) => (
                      <div key={inner.id}>
                        {renderPreviewWidget(inner, true)}
                      </div>
                    ))}
                  </div>
                );
  ...
  return (
    <div className={`min-h-[600px] bg-white ${isMobileView ? "" : ""}`}>
      <div className="mx-auto w-full space-y-0">
        {widgets.map((widget) => (
          <div key={widget.id}>{renderPreviewWidget(widget, false)}</div>
        ))}
      </div>
    </div>
  );
...
function InputLabel({ children }: { children: ReactNode }) {
  return (
    <p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#7a8791]">
      {children}
    </p>
  );
}
  ...
  if (!widget) {
    return (
      <div className="rounded-md border border-[#ccd2d8] bg-white p-4 text-sm text-[#6b7680]">
        Select a widget on the canvas to edit text, colors, spacing, and other
        settings.
      </div>
    );
  }
        ...
        <button
          ...
          className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${
            selectedWidgetId === item.id
              ? "bg-[#2f8fe5] text-white"
              : "bg-white text-[#59636c] hover:bg-[#f6f8fa]"
          }`}
        >
          {index + 1}. {item.label}
        </button>
        ...
        <div className="rounded-md border border-[#ccd2d8] bg-white p-4">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <Input
              type="color"
              label="Text color"
              value={widget.data.color || "#28323b"}
              onChange={(event) =>
                onUpdate(widget.id, (current) =>
                  current.type !== "title"
                    ? current
                    : {
                        ...current,
                        data: { ...current.data, color: event.target.value },
                      },
                )
              }
                ...
                <button
                  ...
                  onClick={() =>
                    onUpdate(widget.id, (current) =>
                      current.type !== "title"
                        ? current
                        : { ...current, data: { ...current.data, alignment } },
                    )
                  }
        ...
        <div className="rounded-md border border-[#ccd2d8] bg-white p-4">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <Input
              type="color"
              label="Text color"
              value={widget.data.color || "#5e6973"}
              onChange={(event) =>
                onUpdate(widget.id, (current) =>
                  current.type !== "text"
                    ? current
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-md border border-[#ccd2d8] bg-white p-4">
            ...
            onChange={(event) =>
              onUpdate(widget.id, (current) =>
                current.type !== "image"
                  ...
                  : {
                      ...current,
                      data: { ...current.data, imageUrl: event.target.value },
                    },
              )
            }
          />
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            />
            <Input
              label="Aspect (`wide`, `square`, `portrait`)"
              value={widget.data.aspect}
              onChange={(event) =>
                onUpdate(widget.id, (current) =>
                  current.type !== "image"
                    ? current
                    : {
                        ...
                        data: {
                          ...
                          aspect: (event.target.value ||
                            "wide") as ImageWidgetData["aspect"],
                        },
                      },
                )
              }
            ...
            onChange={(event) =>
              onUpdate(widget.id, (current) =>
                current.type !== "map"
                  ...
                  : {
                      ...current,
                      data: { ...current.data, address: event.target.value },
                    },
              )
            }
            ...
            onChange={(event) =>
              onUpdate(widget.id, (current) =>
                current.type !== "map"
                  ...
                  : {
                      ...current,
                      data: { ...current.data, embedUrl: event.target.value },
                    },
              )
            }
            className="mt-4"
            ...
            onChange={(event) =>
              onUpdate(widget.id, (current) =>
                current.type !== "leadForm"
                  ...
                  : {
                      ...current,
                      data: { ...current.data, disclaimer: event.target.value },
                    },
              )
            }
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-md border border-[#ccd2d8] bg-white p-4">
          <Input
            type="color"
            label="Divider color"
            value={widget.data.color || "#d7dde3"}
            onChange={(event) =>
              onUpdate(widget.id, (current) =>
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-md border border-[#ccd2d8] bg-white p-4">
          <Input
            label="Spacer height"
            type="number"
            value={String(widget.data.height)}
            onChange={(event) =>
              onUpdate(widget.id, (current) =>
                current.type !== "spacer"
                  ? current
                ...
                onClick={() => {
                  onUpdate(widget.id, (current) => {
                    ...
                    if (numCount > nextItems.length) {
                      ...
                      const removed = nextItems.splice(
                        numCount,
                        nextItems.length - numCount,
                      );
                      removed.forEach((r) => {
                        nextItems[numCount - 1] = [
                          ...nextItems[numCount - 1],
                          ...r,
                        ];
                      });
          ...
          <div className="mt-6 border-t border-[#f1f5f9] pt-6">
            ...
            {widget.data.colCount === 2 ? (
                  ...
                  onChange={(e) => {
                    ...
                    onUpdate(widget.id, (c) =>
                      c.type === "columns"
                        ? {
                            ...c,
                            data: { ...c.data, spans: [span1, 12 - span1] },
                          }
                        : c,
                    );
                  }}
              ...
              <div className="mt-4 grid gap-3 grid-cols-2 sm:grid-cols-4">
                {(
                  widget.data.spans ||
                  Array(widget.data.items.length).fill(
                    Math.floor(12 / widget.data.items.length),
                  )
                ).map((span, idx) => (
                  <div key={idx}>
                      ...
                      onChange={(e) => {
                        ...
                        onUpdate(widget.id, (c) => {
                          ...
                          if (currentSum !== 12) {
                            ...
                            if (idx !== lastIdx) {
                              const otherSum = nextSpans.reduce(
                                (a, b, i) => (i === lastIdx ? a : a + b),
                                0,
                              );
                  ...
                  {widget.data.backgroundImage && (
                    <div className="mt-3 group relative overflow-hidden rounded-lg border border-zinc-200 bg-zinc-50">
                      <img
                        ...
                        className="h-24 w-full object-cover grayscale-[0.2] transition-transform duration-500 group-hover:scale-110"
                      />
          ...
          onChange={(event) =>
            onUpdate(widget.id, (current) =>
              current.type !== "embed"
                ...
                : {
                    ...current,
                    data: { ...current.data, html: event.target.value },
                  },
            )
          }
...
function ToolTile({
  ...
  return (
    ...
    >
      ...
      <span
        className={`text-[9px] font-semibold uppercase tracking-[0.12em] transition-colors ${active ? "text-sky-300" : "text-zinc-500 group-hover:text-zinc-400"}`}
      >
        {label}
      </span>
    </button>
  ...
  if (widget.type === "image") {
    return (
      <div className="absolute left-4 top-4 z-10 w-[min(360px,calc(100%-2rem))] rounded-md border border-[#cfd5db] bg-white/96 p-3 shadow-lg backdrop-blur">
        <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#7a8791]">
          Quick Image Edit
        </p>
        <input
          value={widget.data.imageUrl}
...
export default function LandingPageGenerator() {
  ...
  const generatedCode = useMemo(
    () => generateCodeString(widgets, realtorId),
    [widgets, realtorId],
  );
  ...
  const applyDrop = (payload: DragPayload, target: DropPosition) => {
    setWidgets((current) => {
      ...
      if (payload.source === "palette") {
        ...
        const idx = removedFromCurrent.findIndex(
          (w) => w.id === payload.itemId,
        );
        if (idx !== -1) {
      ...
      const sameArray =
        ...
        ((targetIsRoot && removedFromRoot) ||
          (!targetIsRoot &&
            target.parentId === removedFromParentId &&
            target.colIndex === removedFromColIndex));

      const finalIndex =
        sameArray && removedFromIndex < target.index
          ? target.index - 1
          : target.index;
  ...
  const handleDragOver =
    (target: DropPosition) => (event: DragEvent<HTMLElement>) => {
      event.preventDefault();
      ...
      if (target.parentId === "root" || !target.parentId) {
        if (
          ...
          dragPayload.widgetType !== "columns"
        ) {
        ...
        const draggedWidgetId =
          dragPayload.source === "canvas" ? dragPayload.itemId : null;
        if (draggedWidgetId) {
              ...
              if (w.type === "columns") {
                w.data.items.forEach((col) => {
                  const found = col.find(
                    (inner) => inner.id === draggedWidgetId,
                  );
          ...
          if (
            draggedWidget &&
            draggedWidget.type !== "columns" &&
            (target.parentId === "root" || !target.parentId)
          ) {
            event.dataTransfer.dropEffect = "none";
            return;
          }
  ...
  const handleGenerate = async () => {
    if (!subdomain) {
      alert("Please enter a subdomain before generating.");
      return;
    }
  ...
  const previewViewportClassName =
    previewDevice === "mobile" ? "max-w-[430px]" : "max-w-5xl";

  const renderCanvasWidget = (
    widget: CanvasWidget,
    index: number,
    isLast: boolean,
    ...
    return (
      <div key={widget.id} className="space-y-3">
            ...
            {widget.type === "columns" ? (
              <div
                ...
                style={{
                  ...
                  ...(widget.data.backgroundType === "image" &&
                  widget.data.backgroundImage
                    ? { backgroundImage: `url(${widget.data.backgroundImage})` }
                    : {}),
                }}
              >
                      ...
                      {colItems.map((innerWidget, iIdx) => (
                        <div key={innerWidget.id}>
                          ...
                          >
                            <DropIndicator
                              active={
                                dropPosition?.parentId === widget.id &&
                                dropPosition?.colIndex === cIdx &&
                                dropPosition?.index === iIdx
                              }
                            />
                          </div>
                          {renderCanvasWidget(
                        ...
                        >
                          <DropIndicator
                            active={
                              dropPosition?.parentId === widget.id &&
                              dropPosition?.colIndex === cIdx &&
                              dropPosition?.index === colItems.length
                            }
                          />
                        </div>
                      ) : null}
        ...
        {parentId === "root" ? (
          <div
            onDragOver={handleDragOver({ parentId: "root", index: index + 1 })}
            onDrop={handleDrop({ parentId: "root", index: index + 1 })}
          >
            <DropIndicator
              active={
                dropPosition?.parentId === "root" &&
  ...
  return (
    <div
      className={`-mx-4 sm:-mx-6 lg:-mx-8 -mt-4 sm:-mt-6 lg:-mt-8 ${canvasView === "preview" ? "" : "space-y-6 p-4 sm:p-6 lg:p-8"}`}
    >
      <div
        className={`overflow-hidden transition-all duration-500 ${canvasView === "preview" ? "rounded-none border-0" : "rounded-2xl border border-zinc-200/80 shadow-[0_24px_80px_-12px_rgba(0,0,0,0.25)]"} bg-zinc-50`}
      >
        <div
          className={`grid min-h-[calc(100vh-56px)] gap-0 transition-all duration-500 ${canvasView === "preview" ? "grid-cols-1" : "lg:grid-cols-[72px_280px_minmax(0,1fr)] 2xl:grid-cols-[72px_300px_minmax(0,1fr)]"}`}
        >
          {canvasView !== "preview" && (
          ...
          {canvasView !== "preview" && (
            <aside className="overflow-y-auto border-b border-zinc-200 bg-zinc-50 p-4 sm:p-5 lg:max-h-[calc(100vh-64px)] lg:border-b-0 lg:border-r">
              ...
              <Input
                label="Realtor ID"
                value={realtorId}
                onChange={(event) => setRealtorId(event.target.value)}
                className="mb-5 border-zinc-200 bg-white text-sm"
              />
                  ...
                  <div>
                    ...
                    <div className="mt-2.5 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-2">
                      {paletteGroups.basic.map((item) => (
                        ...
                        >
                          <div className="flex h-8 w-8 items-center justify-center text-zinc-400 transition-colors group-hover/tile:text-sky-500">
                            {item.icon}
                          </div>
                          <p className="text-[10px] font-semibold text-zinc-500 transition-colors group-hover/tile:text-zinc-700">
                            {item.label}
                          </p>
                        </button>
                      ))}
          ...
          <main className="overflow-hidden bg-white">
            {canvasView === "preview" ? (
              <div className="flex items-center justify-between border-b border-zinc-800 bg-gradient-to-r from-zinc-900 via-zinc-900 to-zinc-800 px-5 py-2.5 text-white">
                ...
                <Button
                  onClick={() => setIsModalOpen(true)}
                  className="bg-sky-500 text-[11px] font-bold shadow-sm hover:bg-sky-400"
                >
                  Export Code
                </Button>
              </div>
              ...
              <div className="border-b border-zinc-200 bg-white px-4 py-2 sm:px-5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1 text-[11px] font-medium">
                    <span className="rounded-md bg-sky-500 px-2 py-1 text-white">
                      Build
                    </span>
                    <span className="rounded-md px-2 py-1 text-zinc-400 hover:text-zinc-600 cursor-pointer transition">
                      Design
                    </span>
                    <span className="rounded-md px-2 py-1 text-zinc-400 hover:text-zinc-600 cursor-pointer transition">
                      Pages
            ...
            <div
              className={`transition-all duration-700 ${canvasView === "preview" ? "bg-[#0f1115] min-h-[calc(100vh-140px)] flex items-center justify-center p-8" : "bg-zinc-50 p-3 sm:p-5"}`}
            >
              <div
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <div
                      className={`${previewDevice === "mobile" && canvasView === "preview" ? "relative h-full w-full overflow-y-auto overflow-x-hidden bg-white scroll-smooth custom-scrollbar" : ""}`}
                    >
                        ...
                        <div className="relative min-h-full">
                          {widgets.length === 0 ? (
                            <div className="flex h-full min-h-[600px] flex-col items-center justify-center p-10 text-center bg-zinc-50">
                              ...
                              <h3 className="text-base font-bold text-zinc-800 uppercase tracking-wider">
                                Canvas Empty
                              </h3>
                              <p className="mt-2 text-xs leading-relaxed text-zinc-400 font-medium">
                                Add widgets in the builder to see them appear on
                                this device preview.
                              </p>
                            </div>
                          ) : (
                          ...
                          <div className="flex min-h-[520px] items-center justify-center px-6 text-center">
                            <div>
                              ...
                              <p className="mt-1.5 text-xs text-zinc-400">
                                Add widgets from the builder to see your site
                                here.
                              </p>
                            </div>
                          </div>
                            ...
                            {widgets.length === 0 ? (
                              ...
                              >
                                ...
                                <p className="text-sm font-semibold text-zinc-600">
                                  Drop a Columns layout to start
                                </p>
                                <p className="mt-1.5 max-w-xs text-xs leading-relaxed text-zinc-400">
                                  Drag a Columns widget from the sidebar, or
                                  click it to add one automatically.
                                </p>
                              </div>
                            ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-2xl">
            ...
            <p className="mb-5 text-sm text-zinc-500">
              Please enter the subdomain for this landing page.
            </p>
            <Input
              autoFocus
              label="Subdomain"
              ...
              placeholder="e.g. my-awesome-listing"
              className="mb-6 bg-white border-zinc-200"
            />
            <div className="flex justify-end gap-3">
              ...
              <Button
                ...
                disabled={!subdomain || isGenerating}
              >
